...
void  dieCauseMultipleClients(){
  printf("\n%s\n%s\n%s\n%s\n%s\n%s\n", 
	 ...
	 "###################################################",
	 "For the test with UDP in lab 5, you need to run ONE SERVER PER CLIENT",
  ...
  while(1) {
    ...
    if (buf[0]==0) {
      /* If the packet starts with 0, this means that this was the
         ...
         summary, reset the counters and sends an ack to the client */
      if(sent>0) {
	printf("packet received = %d / %d sent: %2.3f%% loss\n", nb_packets, sent, 100*(1-((double)nb_packets)/sent));
      }
	...
	printf("%5.1fs - received:%6d/ sent:%6d pkts (loss%7.3f%%),%7.1f kbit/s\n", 
