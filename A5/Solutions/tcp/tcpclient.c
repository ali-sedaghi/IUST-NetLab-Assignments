...
int main(int argc, char **argv) {
  ...
  for (;;) 
	...
	if (!(errno == EWOULDBLOCK) ) { // if error is not caused by TIMEOUT, we die.
      ...
      if (info.tcpi_snd_cwnd != prev_cwnd || now - prev_time >= 1.0) 
	{
	  if (now - prev_time >= 1.0) 
	    {
	      int current_buffer_size; 
	      ioctl(sockfd, TIOCOUTQ, &current_buffer_size);
	      cr = (double) (bytes_written-current_buffer_size+buffer_size) * 8.0 / 1024.0 / (now - prev_time);
	      total_rate = (double) (total_bytes_written-current_buffer_size) * 8.0 / 1024.0 / (now - start_time);
	      buffer_size = current_buffer_size;
	      /* ignore slowstart in EWMA */
	      if (now - start_time < 5.0) rate = cr;
	      else                        rate = 0.9 * rate + 0.1 * cr;
	      prev_time = now;
	      bytes_written = 0;
	      
	    }
	  printf("%5.1f:%8.1fkbps avg (%7.1f[inst],%7.1f[mov.avg]) cwnd %2i rtt%5.1fms\n",
		 now - start_time, 
